...
| 参数 | 值 |
|------|-----|
| 数据根目录 (data_path) | {getattr(args, 'data_path', getattr(args, 'data_root', 'N/A'))} |
| 序列长度 (sequence_length) | {getattr(args, 'sequence_length', 'N/A')} |
| 最大序列数 (max_sequences) | {getattr(args, 'max_sequences', 'N/A')} |

## 显存管理参数

| 参数 | 值 |
|------|-----|
| 显存清理频率 (cleanup_freq) | {getattr(args, 'cleanup_freq', 'N/A')} |
| 显存阈值 (memory_threshold) | {getattr(args, 'memory_threshold', 'N/A')} GB |
| 梯度累积步数 (accumulation_steps) | {getattr(args, 'accumulation_steps', 'N/A')} |

## 运行模式

| 模式 | 状态 |
|------|------|
| 干运行 (dry_run) | {getattr(args, 'dry_run', 'N/A')} |
| 仅测试 (test_only) | {getattr(args, 'test_only', 'N/A')} |
| 调试模式 (debug) | {getattr(args, 'debug', 'N/A')} |

## 设备参数

| 参数 | 值 |
|------|-----|
| 禁用CUDA (no_cuda) | {getattr(args, 'no_cuda', 'N/A')} |
| 设备 (device) | {getattr(args, 'device', 'N/A')} |
| 多GPU (multi_gpu) | {getattr(args, 'multi_gpu', 'N/A')} |
| GPU IDs | {getattr(args, 'gpu_ids', 'None')} |

## 显存分析

| 参数 | 值 |
|------|-----|
| 启用显存分析 (enable_memory_profile) | {getattr(args, 'enable_memory_profile', 'N/A')} |
| 显存分析输出 (memory_profile_output) | {getattr(args, 'memory_profile_output', 'N/A')} |

## Rerun可视化

| 参数 | 值 |
|------|-----|
| 启用Rerun可视化 (enable_rerun_viz) | {getattr(args, 'enable_rerun_viz', 'N/A')} |
| 可视化目录 (rerun_viz_dir) | {getattr(args, 'rerun_viz_dir', 'N/A')} |
| 可视化频率 (rerun_viz_freq) | {getattr(args, 'rerun_viz_freq', 'N/A')} |
