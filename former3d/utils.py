...
def cam2ned(cam_in_world):
    ...
    cam_in_ned = T.dot(cam_in_world).dot(T_inv)
    
    return np.array(cam_in_ned)


def read_traj_file(file_path):  
    """  
    Read trajectory file from TartanAir dataset.  
...
def pose_quat2mat(pose):  
    """  
    Convert pose from quaternion format [tx, ty, tz, qx, qy, qz, qw] to 4x4 transformation matrix.  
    """  
    import numpy as np  
    from scipy.spatial.transform import Rotation  
      
    # Extract translation and quaternion  
    if len(pose) == 8:  # If timestamp is included  
        tx, ty, tz = pose[1:4]  
