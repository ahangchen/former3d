...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('fixed_training_debug.log')
    ]
)
...
try:
    ...
    print(f"⚠️ 无法导入设备一致性工具: {e}")
    print("⚠️ 将继续使用基本设备管理，可能出现设备不匹配错误")
...
def train_epoch(model, dataloader, criterion, optimizer, device):
    ...
    for batch_idx, batch in enumerate(dataloader):
        # 使用设备一致性工具确保整个batch在正确设备上
        if DEVICE_CONSISTENCY_AVAILABLE:
            logger.debug(f"训练批次 {batch_idx}: 应用设备一致性到 {device}")
            batch = move_to_device(batch, device)
            # 检查设备一致性
            device_info = batch_device_check(batch, stage=f"训练批次_{batch_idx}")
            if device_info['device_mismatch']:
                logger.error(f"训练批次 {batch_idx}: 设备不匹配检测到!")
        else:
            logger.warning(f"训练批次 {batch_idx}: 设备一致性工具不可用，使用基本设备移动")
            # 基本设备移动
            rgb_images = batch['rgb_images'].to(device)
            tsdf_gt_raw = batch['tsdf'].to(device)
            # 更新batch以保持一致性
            batch['rgb_images'] = rgb_images
            batch['tsdf'] = tsdf_gt_raw
        
        # 获取数据（现在整个batch都在正确设备上）
        rgb_images = batch['rgb_images']
        tsdf_gt_raw = batch['tsdf']  # [batch, 1, H, W, D]
        ...
        # 使用第一帧进行训练（简化）
        input_3d = prepare_input_data(rgb_images, tsdf_gt, frame_idx=0, device=device)
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            if DEVICE_CONSISTENCY_AVAILABLE:
                batch = move_to_device(batch, device)
                # 检查设备一致性
                batch_device_check(batch, stage=f"验证批次_{batch_idx}")
            
            # 获取数据（现在整个batch都在正确设备上）
            rgb_images = batch['rgb_images']
            tsdf_gt_raw = batch['tsdf']
