#!/usr/bin/env python3
"""
修复spconv SparseConvTensor错误
错误：you can't set feature directly, use 'x = x.replace_feature(your_new_feature)'
"""

import os

def find_spconv_errors():
    """查找可能导致spconv错误的代码"""
    print("查找spconv相关错误...")
    
    # 检查StreamSDFFormerIntegrated模型
    ...
    if os.path.exists(file_path):
        ...
        found_issues = []
        for pattern in spconv_patterns:
            if pattern in content:
                found_issues.append(pattern)
                print(f"⚠️ 发现可能的问题模式: {pattern}")
        
        if found_issues:
            print(f"\n发现{len(found_issues)}个可能的问题")
            return True
        else:
            print("✅ 未发现明显的spconv直接赋值问题")
            return False
    else:
...
'''
    ...
    script_path = "workaround_training.py"
    with open(script_path, 'w') as f:
        f.write(script_content)
    
    os.chmod(script_path, 0o755)
    print(f"✅ 绕过脚本已创建: {script_path}")
...
def main():
    """主函数"""
    print("="*80)
    print("修复spconv SparseConvTensor错误")
    print("="*80)
    
    # 查找错误
    has_issues = find_spconv_errors()
    
    if has_issues:
        print("\n⚠️ 发现可能的spconv问题")
        print("建议:")
        print("1. 检查StreamSDFFormerIntegrated模型中的稀疏张量操作")
        print("2. 确保使用.replace_feature()而不是直接赋值")
        print("3. 或者使用简化模型绕过问题")
    else:
        print("\n✅ 未发现明显的代码问题")
        print("错误可能发生在模型内部或依赖库中")
    ...
    print("修复方案")
    print("="*80)
    print("1. 短期方案: 使用简化模型绕过spconv错误")
    print(f"   运行: python {script_path}")
    print("2. 长期方案: 修复StreamSDFFormerIntegrated中的spconv操作")
    print("   需要检查模型内部代码")
    print("\n🚀 现在可以尝试运行绕过脚本验证训练流程!")
