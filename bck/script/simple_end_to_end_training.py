...
def create_simple_model():
    ...
    try:
        ...
        logger.info("创建简化的StreamSDFFormerIntegrated模型...")
...
def train_epoch(model, dataloader, optimizer, loss_fn, config, epoch):
    ...
    for batch_idx, batch in enumerate(dataloader):
        try:
            # 移动到设备
            images = batch['rgb_images'].to(device)  # [B, F, C, H, W]
            ...
            for frame_idx in range(images.shape[1]):
                # 提取当前帧
                current_images = images[:, frame_idx:frame_idx+1]  # [B, 1, C, H, W]
                current_poses = poses[:, frame_idx:frame_idx+1]    # [B, 1, 4, 4]
                
                # 重塑为模型期望的形状
                current_images_reshaped = current_images.squeeze(1)  # [B, C, H, W]
                current_poses_reshaped = current_poses.squeeze(1)    # [B, 4, 4]
...
def main():
    """主训练函数"""
    logger.info("开始简化的端到端训练...")
    
    # 创建数据集
    dataloader, config = create_dataset()
