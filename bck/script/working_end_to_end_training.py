...
def create_working_model():
    ...
    try:
        ...
        logger.info("创建可工作的StreamSDFFormerIntegrated模型...")
...
def train_epoch_simple(model, dataloader, optimizer, loss_fn, config, epoch):
    """简单的训练epoch"""
    model.train()
    total_loss = 0
    num_batches = 0
        ...
        try:
            ...
            if 'sdf' in output:
                ...
                if len(pred_sdf.shape) == 2:  # [N, 1]
                    ...
                    raise ValueError(f"不支持的pred_sdf形状: {pred_sdf.shape}")
...
def main():
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # 训练一个epoch
        train_loss = train_epoch_simple(model, dataloader, optimizer, loss_fn, config, epoch)
